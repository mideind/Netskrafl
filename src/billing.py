...
def request_valid(
    ...
    try:
        dt = datetime.strptime(xsc_date, "%Y-%m-%d %H:%M:%S").replace(tzinfo=UTC)
    except ValueError:
        # Invalid date/time
        return False
...
def cancel_plan(user: User) -> bool:
    ...
    try:
        ...
        user.add_transaction("", "salescloud", "cancel")
...
def handle(request: Request, uid: str) -> ResponseType:
    ...
    if (
        ...
        if status == "true":
            ...
            user.add_transaction("friend", "salescloud", "enable")
            ...
            # Disable subscription, remove friend status
            user.add_transaction("", "salescloud", "disable")
